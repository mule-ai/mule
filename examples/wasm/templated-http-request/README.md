...
This example demonstrates how to make HTTP requests from a WASM module using the host functions provided by the Mule runtime, with added support for templating using the `{{.MESSAGE}}` variable.
...
5. Use template variables in request data

## Features

This module extends the basic HTTP request functionality by adding template support. You can use `{{.MESSAGE}}` anywhere in your request data, and it will be replaced with the value from the `prompt` field in the input.
...
4. Only the `{{.MESSAGE}}` template variable is supported
...
1. Compile to WASM:
   ```bash
   GOOS=wasip1 GOARCH=wasm go build -o templated-http-request.wasm .
   ```
...
3. The module will make an HTTP POST request with the data where `{{.MESSAGE}}` is replaced with "Hello, world!" and return the success status
