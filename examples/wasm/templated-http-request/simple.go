...
// processTemplate replaces template variables in the data with actual values
func processTemplate(data interface{}, prompt string) interface{} {
	if data == nil {
		return nil
	}
	...
	if err != nil {
		// If we can't marshal, return as is
		return data
	}

	// Convert to string and replace template variables
	dataStr := string(dataBytes)

	// Replace {{.MESSAGE}} with the prompt
	dataStr = string(bytes.ReplaceAll([]byte(dataStr), []byte("{{.MESSAGE}}"), []byte(prompt)))

	// Try to unmarshal back to interface{}
	var result interface{}
	if err := json.Unmarshal([]byte(dataStr), &result); err != nil {
		// If we can't unmarshal, return the string
		return dataStr
	}
...
func main() {
	...
	if input.Data != nil {
		// Process template variables in the data
		processedData := processTemplate(input.Data, input.Prompt)

		bodyBytes, err := json.Marshal(processedData)
		if err != nil {
